...
3. **Preservare il lavoro esistente.** Leggere `AGENTS.md`, controllare `git status` e il diff prima di intervenire. Non cancellare o includere nei propri commit modifiche altrui. Usare branch `codex/<argomento>`; il push su `main` può attivare la produzione.
